...
user_dir = Path(user_config_dir("ipe-figures", "Castel"))
...
def watch(daemon):
    ...
    if daemon:
        daemon = Daemonize(app='ipe-figures',
                           pid='/tmp/ipe-figures.pid',
                           action=watcher_cmd)
        daemon.start()
...
def maybe_recompile_figure(filepath):
    ...
    ipe_version = subprocess.check_output(['ipe', '--version'], universal_newlines=True)
    log.debug(ipe_version)

    # Convert
    # - 'Inkscape 0.92.4 (unknown)' to [0, 92, 4]
    # - 'Inkscape 1.1-dev (3a9df5bcce, 2020-03-18)' to [1, 1]
    # - 'Inkscape 1.0rc1' to [1, 0]
    ipe_version = re.findall(r'[0-9.]+', ipe_version)[0]
    ipe_version_number = [int(part) for part in ipe_version.split('.')]

    # Right-pad the array with zeros (so [1, 1] becomes [1, 1, 0])
    ipe_version_number= ipe_version_number + [0] * (3 - len(ipe_version_number))

    # Tuple comparison is like version comparison
    if ipe_version_number < [1, 0, 0]:
        command = [
            'ipe',
            '--export-area-page',
            '--export-dpi', '300',
...
def create(title, root):
    ...
    """
    title = title.strip()
    file_name = title.replace(' ', '-').lower() + '.ipe'
